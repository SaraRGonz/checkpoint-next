...
export default function RegisterPage() {
    ...
    return (
        <div className="relative flex flex-col items-center justify-center min-h-[80vh] py-12 px-4 overflow-hidden">
            ...
            <div 
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-150 h-150 bg-accent/10 blur-[120px] 
                rounded-full pointer-events-none -z-10">
            </div>
            
            <div 
                className="absolute bottom-1/3 right-1/3 w-75 h-75 bg-tertiary/10 blur-[100px] rounded-full pointer-events-none -z-10">
            </div>

            <div 
                className="relative w-full max-w-md p-8 bg-gray-900/60 backdrop-blur-xl border border-gray-800 shadow-2xl 
                ...
                <form onSubmit={handleRegister} noValidate className="w-full flex flex-col gap-5">
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full relative group overflow-hidden bg-accent/10 text-accent font-rajdhani font-bold text-lg 
                            tracking-widest uppercase py-3 rounded border border-accent/50 hover:bg-accent hover:text-black transition-all 
                            duration-300 disabled:opacity-50 disabled:cursor-not-allowed mt-2"
                    >
                ...
                <p className="text-center text-sm text-gray-400 mt-8">
                    ...
                    className="text-primary hover:text-primary/80 font-semibold hover:underline transition-colors">Sign in</Link>
                </p>
            </div>
        </div>
    );
